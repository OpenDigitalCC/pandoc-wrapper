...
# in dist/bg-guides/ showing the page(s) with light hatching over the zones the
# template fills with content. A designer making a background image uses it as a
# mask layer: keep focal points in the clear (un-hatched) areas.
#
...
for g in featured report slides beamer; do
    ...
    if ( cd "$SRC" && xelatex -interaction=nonstopmode -halt-on-error -output-directory="$WORK" "$g.tex" >/dev/null 2>&1 \
        ...
        printf '  %-10s -> dist/bg-guides/%s-background-guide.pdf\n' "$g" "$g"
    else
